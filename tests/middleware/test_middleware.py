...
def test_validate_empty_problem_id(submission_data: Middleware):
    ...
    expected_error_message = "Missing or empty problem_id field."
    with pytest.raises(RequestFormatError, match=expected_error_message):
        submission_data.empty_data()


def test_validate_empty_submission_id(submission_data: Middleware):
    """verify that input is not empty"""
    submission_data.submission_id = ""
    expected_error_message = "Missing or empty submission_id field."
    with pytest.raises(RequestFormatError, match=expected_error_message):
        submission_data.empty_data()
